...
class ShortestPathTests(unittest.TestCase):
    ...
    def test_branching_graph_selects_shortest_geometric_zone_order(self):
        _, _, _, path, end_pos = self._sept_iles_gaspe_path()

        sol = path.compute(end_pos)

        self.assertEqual(sol.zone_sequence, [0, 1, 2, 3])
        self.assertAlmostEqual(sol.total_distance, 206.1636121564717, places=6)
        self.assertEqual(len(sol.zone_sequence), sol.waypoints.shape[0] - 1)
    ...
    def test_branching_graph_beats_single_bfs_sequence(self):
        ...
        corner_xy, zone_edges = path._load_zone_geometry()

        bfs_seq = path._build_zone_sequence(
            path._find_zone_containing_point(start),
            path._find_zone_containing_point(end_pos),
        )
        bfs_sol = path._solve_zone_sequence(
            bfs_seq,
            start,
            end_pos,
            corner_xy,
        ...
        self.assertEqual(map_obj.nb_zones, 5)
    ...
    def test_shortest_path_output_feeds_constant_speed_reference(self):
        ...
        ref = build_constant_speed_path_reference(
            ...
            path_zone_ids=sol.zone_sequence,
            itinerary=itinerary,
            states=states,
            map_obj=map_obj,
            ship=ship,
        ...
        self.assertEqual(ref["zone"].shape[1], map_obj.nb_zones)
