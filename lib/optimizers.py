...
class GlobalOptimizer:
    ...
    def optimize(self,
        ...
        for t in range(T_future):
            ...
            constraints += [
                ship_speed_x[t, 0] == ((q[0] - ship_pos[t, 0]) / half_dt_h) * 1000 / 3600,
                ship_speed_y[t, 0] == ((q[1] - ship_pos[t, 1]) / half_dt_h) * 1000 / 3600,

                ship_speed_x[t, 1] == ((ship_pos[t + 1, 0] - q[0]) / half_dt_h) * 1000 / 3600,
                ship_speed_y[t, 1] == ((ship_pos[t + 1, 1] - q[1]) / half_dt_h) * 1000 / 3600,
            ]
                ...
                if interval_sail_fraction[t] > 0.01:
                    ...
                    for z in range(self.map.nb_zones):
                        ...
                        constraints += [
                            wave_resistance[t, h] >=
                            cw[0]
                            + cw[1] * normalized_rel_speed[t, h]
                            + cw[2] * cp.square(normalized_rel_speed[t, h])
                            + cw[3] * cp.power(normalized_rel_speed[t, h], 3)
                            + cw[4] * cp.power(normalized_rel_speed[t, h], 4)
                            + cw[5] * speed_rel_water_x[t, h] / self.ship.info.max_speed
                            + cw[6] * cp.square(speed_rel_water_x[t, h] / self.ship.info.max_speed)
                            + cw[7] * cp.power(speed_rel_water_x[t, h] / self.ship.info.max_speed, 4)
                            + cw[8] * speed_rel_water_y[t, h] / self.ship.info.max_speed
                            + cw[9] * cp.square(speed_rel_water_y[t, h] / self.ship.info.max_speed)
                            + cw[10] * cp.power(speed_rel_water_y[t, h] / self.ship.info.max_speed, 4)
                            - WAVE_BIG_M * (1 - active_zone[z])
                        ]
...
class Global_Continuous:
    ...
    def optimize(self,
        ...
        for t in range(T_future):
            q = crossing_point[t, :]
            constraints += [
                ship_speed_x_split[t, 0] == ((q[0] - ship_pos[t, 0]) / half_dt_h) * 1000 / 3600,
                ship_speed_y_split[t, 0] == ((q[1] - ship_pos[t, 1]) / half_dt_h) * 1000 / 3600,
                ship_speed_x_split[t, 1] == ((ship_pos[t + 1, 0] - q[0]) / half_dt_h) * 1000 / 3600,
                ship_speed_y_split[t, 1] == ((ship_pos[t + 1, 1] - q[1]) / half_dt_h) * 1000 / 3600,
                ship_speed_x[t] == ((ship_pos[t + 1, 0] - ship_pos[t, 0]) / dt_h) * 1000 / 3600,
                ship_speed_y[t] == ((ship_pos[t + 1, 1] - ship_pos[t, 1]) / dt_h) * 1000 / 3600,
            ]
            for h in range(2):
                constraints += [
        ...
        wave_model_future = self.wave_model.thrust_coeffs[:, self.states.timesteps_completed : self.states.timesteps_completed + T_future, :]
        wave_model_nd_future = self.wave_model_nd.thrust_coeffs[:, self.states.timesteps_completed : self.states.timesteps_completed + T_future, :]
        ...
        for t in range(T_future):
            if interval_sail_fraction[t] > 0.01:
                ...
                for z in range(self.map.nb_zones):
                    ...
                    cw2 = wave_model_nd_future[s_plus_1, t, :]
...
class Fixed_Path_TimeSampled_Optimizer:
    ...
    def _precompute_timesampled_weather_models(self, debug=False):
        ...
        wave_amp_ts = np.zeros((1, T_future))
        wave_freq_ts = np.zeros((1, T_future))
        wave_len_ts = np.zeros((1, T_future))
        wave_dir_ts = np.zeros((1, T_future))
        ...
        t0 = int(self.states.timesteps_completed)
        ...
        for t in range(T_future):
            ...
            wind_x_ts[0, t] = self.weather.wind_x[z, gt]
            wind_y_ts[0, t] = self.weather.wind_y[z, gt]

            wave_amp_ts[0, t] = self.weather.mean_wave_amplitude[z, gt]
            wave_freq_ts[0, t] = self.weather.mean_wave_frequency[z, gt]
            wave_len_ts[0, t] = self.weather.mean_wave_length[z, gt]
            wave_dir_ts[0, t] = self.weather.mean_wave_direction[z, gt]

            current_x_ts[t] = self.weather.current_x[z, gt]
            current_y_ts[t] = self.weather.current_y[z, gt]
            irradiance_ts[t] = self.weather.irradiance[z, gt]
        ...
        self.wave_model_ts = WaveModel1D(self.ship, self.wave_model.fit_range)
        self.wave_model_ts.fit_convex_models(
            wave_amp_ts,
            wave_freq_ts,
            wave_len_ts,
            wave_dir_ts,
        ...
        if debug:
            print("[TimeSampled] fitted one wind/wave model per timestep")
    ...
    def optimize(
        ...
        wave_model_future = self.wave_model_ts.thrust_coeffs[0, :, :]

        for t in range(T_future):
            if interval_sail_fraction[t] > 0.01:
                c1 = wind_model_future[t, :]
                cw1 = wave_model_future[t, :]
