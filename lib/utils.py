...
def plot_zones_and_points(
    ...
    """
    Visualize zones and a series of points for the stored convention:
        vals = y*a0 + x*a1 + b >= 0

    ship_pos : array of shape (T, 2)
        Sequence of ship positions
    zone_ineq : array of shape (3, 4, nb_zones)
        Zone inequalities
    eps_in : float
        Tolerance for "point in zone" check
    eps_poly : float
        Tolerance for polygon reconstruction
    """
    ...
    # Evaluate inequalities for all timesteps and zones
    # Result: (T, 4, nb_zones)
    vals_all = (
    ...
    for z in range(nb_zones):
        # Convert stored form to A[x,y] + b >= 0
        A = np.column_stack([
            zone_ineq[1, :, z],  # a_x
            zone_ineq[0, :, z],  # a_y
        ])
